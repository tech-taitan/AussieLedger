...
**Last updated:** 2026-06-02 (v1.2 SHIPPED — Public Hosting + IndexedDB Hardening + PWA + Release Polish; live at https://aussieledger.techtaitan.com; ready for v1.3 scoping)

## Milestones

- ✅ **v1.0** — Phases 1–6 (shipped 2026-05-29) — see [.planning/milestones/v1.0-ROADMAP.md](./milestones/v1.0-ROADMAP.md)
- ✅ **v1.1** — Phases 7–9 (shipped 2026-05-30) — Polish, Closure, and TB Import Rework — see [.planning/milestones/v1.1-ROADMAP.md](./milestones/v1.1-ROADMAP.md)
- ✅ **v1.2** — Phases 10/11/13/14 (shipped 2026-06-02; Phase 12 deferred to v5) — Public Hosting + IndexedDB Hardening + PWA + Release Polish — see [.planning/milestones/v1.2-ROADMAP.md](./milestones/v1.2-ROADMAP.md)
- 📋 **v2.0** — Standalone desktop app + file-backed SQLite + hard network sandbox — research at [.planning/future-milestones/v2.0-standalone-app/](./future-milestones/v2.0-standalone-app/) (deferred)
...
<details>
<summary>✅ v1.2 (Phases 10/11/13/14) — SHIPPED 2026-06-02 (Phase 12 deferred to v5)</summary>

- [x] Phase 10: Public Build + CI/CD (Cloudflare→Vercel pivot) — completed 2026-06-01 (HOST-04 closed early)
- [x] Phase 11: IndexedDB Hardening (2/2 plans) — completed 2026-06-01
- [~] Phase 12: User-Supplied AI Key + Direct-Browser Gemini — **DEFERRED to v5** (2026-06-01); AI-01/02 in Future Requirements
- [x] Phase 13: PWA Wrapper (2/2 plans) — completed 2026-06-02
- [x] Phase 14: Release Polish (3/3 plans) — completed 2026-06-02

</details>

### v1.3 (Pending — scoped 2026-06-02)
...
- [~] **Phase 15: Code Polish** — POL-CODE-01..05 (POL-CODE-06 deferred) — repo-visibility-fix · legacy-migration-demo-guard · `<button>`-in-`<button>` Sidebar refactor · entity-aware tax-nav · Edit Entity Details in Settings — 2/2 plans complete (15-1 done; 15-2 done); UAT walkthrough doc landed at `.planning/phases/15-code-polish/15-UAT.md` (5 scenarios / 24 steps); awaiting user UAT signoff.
- [~] **Phase 16: Docs Polish** — POL-DOCS-02 shipped (persona-segmented README sections); POL-DOCS-01 (real /demo screenshot) DEFERRED to v1.4 per user `skip-screenshot` decision — root cause diagnosed as PWA stale-cache (not a `seedDemoData()` regression); user re-pivoted on 2026-06-03 to skip the followup capture entirely and defer in-milestone. PWA stale-cache hardening is a separate v1.4 investigation todo. 1/1 plans complete; UAT walkthrough doc landed at `.planning/phases/16-docs-polish/16-UAT.md` (2 scenarios / 12 steps for POL-DOCS-02; POL-DOCS-01 marked deferred out of UAT scope); awaiting user UAT signoff. After both UAT signoffs, v1.3 ready for `/gsd:audit-milestone v1.3` (6 of 7 active reqs closed; POL-DOCS-01 deferred to v1.4 with attached PWA-stale-cache + screenshot-capture-brief refs).
...
**Plans:** 2 plans (both complete; checker PASS round 2 — B-1 + R-2 resolved):
- 13-1 (COMPLETE 2026-06-01): Icons + vite-plugin-pwa config + manifest + pwa-config structural test — 3 tasks, +30 SPA GREEN
- 13-2 (COMPLETE 2026-06-02): UpdateBanner + useUpdateBanner hook + App.tsx wiring + Lighthouse smoke — 3 tasks (Wave 2), +14 SPA GREEN, user manual smoke approved (Lighthouse Installable + PWA Optimized both PASS; npm-run-dev SW absence verified; end-to-end UpdateBanner update flow verified incl. B-1 sessionStorage-persists-within-tab lock)
...
**Depends on:** Phases 10, 11, 12, 13 (all must be complete — README needs the live custom domain; `/demo` route needs hosting verified; `/privacy` page belongs with a complete feature set; first-visit UX benefits from IDB hardening and PWA being in place). HOST-04 already closed early in Phase 10's Vercel pivot — Phase 14 effective scope is POL-01..04.

**Requirements:** POL-01, POL-02, POL-03, POL-04 (HOST-04 closed early in Phase 10)

**Success Criteria** (what must be TRUE):
1. ~~The custom domain (e.g. `aussieledger.app` or `aussieledger.com.au`) resolves to the Cloudflare Pages deployment over HTTPS with auto-renewing cert; the README top-of-fold "Try the live demo at {URL}" link points to this custom domain, not the `.pages.dev` default~~ — already done in Phase 10 (Vercel pivot configured `aussieledger.techtaitan.com` at project setup; HOST-04 closed)
...
4. The `/privacy` page is accessible from every page footer and lists the trust signals: no third-party scripts, no cookies, no analytics, no server-side storage, AI status honestly notes v5-deferral, Apache 2.0 license with repo link
5. The README is rewritten with top-of-fold "Try the live demo at https://aussieledger.techtaitan.com" + 1-line elevator pitch + screenshot placeholder; quick-start covers both (1) try the demo and (2) clone + self-host; `npm run dev:full` for the Express+SQLite shape is documented; `server/` continuity is preserved and documented as the small-firm VPS path
