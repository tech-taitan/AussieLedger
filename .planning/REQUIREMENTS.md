...
- [x] **POL-CODE-01 (repo-visibility-fix)**: ~~Either (a) the GitHub repository at `github.com/tech-taitan/AussieLedger` is flipped to public via GitHub Settings → Danger Zone → Change visibility, OR (b) all anonymous-broken repo URL references are stripped/replaced in `README.md:91` + `src/components/PrivacyPage.tsx:55,59,89` with an honest "source available on request" line. User decision at execution-discussion time. Closes v1.2 audit RED.~~ — **COMPLETE 2026-06-02 (Plan 15-1 Task 1).** Option (a) selected — GitHub repo flipped public via Settings → Danger Zone → Change visibility (checkpoint:human-action gate). Anonymous probe of api.github.com/repos/tech-taitan/AussieLedger returns 200 with private=false, visibility=public, license.spdx_id=Apache-2.0 (was 404). PrivacyPage AI bullet honesty check was a NO-OP — Phase 14-2 had already shipped the honest wording. 3 metadata variances logged for v1.4 polish (user-chosen description; topics empty; homepage at Vercel default instead of custom domain). No commit produced for Task 1's code branch (no-source-diff outcome by design). v1.2-audit-RED #1 CLOSED.
- [x] **POL-CODE-02 (legacy-migration-demo-guard)**: ~~`src/storage/local.ts` constructor (line 117-122) skips `migrateLegacyLocalStorage(this)` when `this.dbName === DB_NAME_DEMO`. Prevents the v1.2-audit-AMBER scenario where a pre-Phase-11 user who lands on `/demo` BEFORE ever loading `/` has their legacy localStorage data migrated INTO the demo DB and legacy keys cleared. Unit test: `new LocalAdapter(DB_NAME_DEMO)` + pre-populated legacy keys → demo DB stays empty + legacy keys NOT cleared.~~ — **COMPLETE 2026-06-02 (Plan 15-1 Task 2; commits 1e2a91b RED + f15cdae GREEN).** Guard implementation landed at the TOP of `migrateLegacyLocalStorage(adapter)` in `src/storage/legacy-migration.ts` (NOT inside the LocalAdapter constructor — single source of truth at the migration entry point; reads Phase 14 duck-typed `adapter.getDbName()` accessor so StorageAdapter FINAL preserved). 2 new tests GREEN: `src/storage/__tests__/legacy-migration-demo-guard.test.ts` — (1) demo-skips-migration (demo DB stays empty + 4 legacy keys preserved) + (2) prod-still-migrates regression guard (entity lands in prod DB + 4 legacy keys cleared). All 5 existing `src/storage/__tests__/legacy-migration.test.ts` tests stay GREEN. v1.2-audit-AMBER #2 CLOSED.
- [x] **POL-CODE-03 (button-in-button-sidebar-refactor)**: ~~Sidebar's NavButton anomaly badge (`src/components/shell/Sidebar.tsx:85-95`) restructured so it does NOT render `<button>` nested inside `<button>`. Either (a) anomaly badge becomes `<span role="button" tabIndex={0}>` with keyboard handling (Enter + Space → click), or (b) NavButton becomes a `<div role="link">` and the badge stays a real `<button>`. React console warning gone. All existing Sidebar tests still GREEN; new keyboard-accessibility test added.~~ — **COMPLETE 2026-06-03 (Plan 15-2 Task 1; commits 7394346 RED + 0330e18 GREEN).** Option (a) selected — badge swapped from `<button>` to `<span role="button" tabIndex={0}>` with `onKeyDown` handler dispatching `onBadgeClick` on Enter + Space (with mandatory `e.preventDefault()` on Space to prevent default page-scroll). Tailwind classes preserved verbatim plus `cursor-pointer` added. `data-testid` + `aria-label` preserved verbatim. React's nested-interactive-elements console warning silenced. Existing S.8 + S.9 expectations updated in same RED commit (BUTTON → SPAN + role assertion). +2 keyboard-a11y tests K.1 (Enter) + K.2 (Space). 16 Sidebar tests GREEN total.
- [x] **POL-CODE-04 (entity-aware-tax-nav)**: ~~Sidebar tax-section nav entries (`setView('tax-return' | 'company-tax' | 'trust-tax' | 'bas-ias')` in `src/components/shell/Sidebar.tsx`) filter by `activeEntity?.type`. Mapping: Individual / Sole Trader → Tax Assist (Form I) only; Company → Company Tax (Form C) only; Trust → Trust Tax (Form T) only; Partnership → Partnership Tax (Form P) only. BAS/IAS stays visible regardless (universal across entity types). When no active entity selected (master-dashboard), all 4 tax-section entries hidden. Tests: each entity type renders the correct subset; no-entity case hides all 4.~~ — **COMPLETE 2026-06-03 (Plan 15-2 Task 2; commits f67a0c3 RED + 3beb530 GREEN).** Entity-scoped block now filters the 3 specialised tax sections via 3 inline conditional renders + 1 universal BAS/IAS render. Final mapping: `(Individual || SoleTrader) → Tax Assistant`; `Company → Company Tax`; `Trust → Trust Tax`; `Partnership → none of the 3 specialised sections` (no Form P view exists today; deferred). BAS/IAS UNCONDITIONAL inside the activeEntity block (universal across types per CONTEXT lock — GST applies regardless of type). Demo-seed.ts discriminator confirmed at execution: `type: 'SoleTrader'` (line 38) — SoleTrader consolidated with Individual via OR clause. +11 tests added (ET.1-6 = 5 type branches + no-entity; ET.7 `it.each` parametric × 4 BAS-universal; ET.8 Entity Dashboard universal proof). 27 Sidebar tests GREEN total.
- [x] **POL-CODE-05 (edit-entity-details-in-settings)**: ~~A new "Active Entity" section added to `src/components/Settings.tsx` (after existing "Mode" and "Primary Entity" sections). Section shows active entity name + an "Edit Entity Details" button that opens `EntityForm` for the active entity. When no active entity selected, section shows "No active entity selected" prompt. The existing `ViewRouter.tsx:179` "Edit Entity Details" header button stays unchanged (duplicate, NOT move).~~ — **COMPLETE 2026-06-03 (Plan 15-2 Task 3; commits 4312158 RED + b9f6005 GREEN).** New 4th `<section>` "Active Entity" inserted between Primary Entity (lines 54-82) and First-Run Prompt (line 84). Section ALWAYS renders. Entity-present branch: name + `(type)` chip + `Edit Entity Details` button (data-testid `settings-edit-active-entity`). Entity-absent branch: `No active entity selected` + hint `Select an entity from the Master Dashboard to edit` (no button). SettingsProps widened with `activeEntity?: Entity` + `onEditActiveEntity?: () => void`. ViewRouter widens Settings invocation to pass `activeEntity` (reused from existing line-594 const) + `onEditActiveEntity={() => setView('edit-entity')}` (closure inline). ViewRouter:179 header button UNCHANGED (duplicate, not move — verified). App.tsx no-source-diff (wiring purely inside ViewRouter). +3 tests (SET.5/6/7). 7 Settings tests GREEN total.
...
| Req | Phase | Status |
|-----|-------|--------|
| POL-CODE-01 | Phase 15 Plan 15-1 | COMPLETE 2026-06-02 (GitHub repo flipped public; PrivacyPage no-op; v1.2-audit-RED #1 closed) |
| POL-CODE-02 | Phase 15 Plan 15-1 | COMPLETE 2026-06-02 (legacy-migration demo-DB guard + 2 tests; commits 1e2a91b/f15cdae; v1.2-audit-AMBER #2 closed) |
| POL-CODE-03 | Phase 15 Plan 15-2 | COMPLETE 2026-06-03 (Sidebar badge span role=button + keyboard handler; commits 7394346/0330e18; v1.2-audit-AMBER #3 closed) |
| POL-CODE-04 | Phase 15 Plan 15-2 | COMPLETE 2026-06-03 (entity-type-aware tax-nav filter; SoleTrader = Individual; Partnership = BAS only; BAS universal; commits f67a0c3/3beb530) |
| POL-CODE-05 | Phase 15 Plan 15-2 | COMPLETE 2026-06-03 (Settings Active Entity section + ViewRouter wiring; ViewRouter:179 preserved; App.tsx no-diff; commits 4312158/b9f6005) |
