...
- [x] **PWA-01**: `vite-plugin-pwa@^1.3.0` integrated into `vite.config.ts` with `generateSW` strategy (NOT `injectManifest` — no custom SW logic needed) and `registerType: 'prompt'` (NOT `'autoUpdate'` — avoids force-reload mid-form). `skipWaiting: true` + `clientsClaim: true` + `cleanupOutdatedCaches: true` all configured to prevent the stale-cache trap. `manifest.json` includes `name`, `short_name`, `description`, `icons` (192px + 512px PNG), `theme_color`, `background_color`, `display: 'standalone'`, `start_url: '/'`. Service worker only activates in production builds — zero impact on `npm run dev`. Update banner appears when SW detects a new version: "A new version of AussieLedger is available." with Update + Later buttons; explicit user click required (no auto-reload). **Complete 2026-06-02:** install path landed in Plan 13-1 (vite-plugin-pwa@^1.3.0 + @resvg/resvg-js@^2.6.2 in devDependencies; pwaOptions named export in vite.pwa-options.ts locks all three stale-cache flags + registerType:'prompt' + devOptions.enabled:false + injectRegister:false; 5 PNG icons committed in public/; index.html has apple-touch-icon + theme-color; dist/sw.js + workbox-*.js + manifest.webmanifest emit on every build; 3 contract tests = 30 GREEN). Update flow landed in Plan 13-2 (useUpdateBanner hook wires registerSW({onNeedRefresh, onOfflineReady}) once per App mount via runtime-variable-indirected dynamic import + try/catch + __setRegisterSWForTests injectable seam; UpdateBanner component renders verbatim CONTEXT-locked copy with fixed top-0 z-50 positioning; sessionStorage snooze under 'aussieledger:pwa-update-snoozed' = 'true' (per-session, re-fires next session); 12 new behaviour tests GREEN; App.tsx wiring is +2 lines, 0 deletions). User-verified manual smoke 2026-06-02: Lighthouse Installable + PWA Optimized both PASS in Chrome at http://localhost:4174; npm run dev shows EMPTY Service Workers list at localhost:3000; end-to-end UpdateBanner flow PASS incl. B-1-locked sessionStorage-persists-within-tab snooze behaviour (banner stays hidden after same-tab reload; re-fires only after closing+reopening tab or in private/incognito window).
...
- [ ] **POL-02**: `/demo` route loads a separate IndexedDB database named `'aussieledger-demo'` (NOT the production `'aussieledger'` namespace) pre-seeded with an anonymised sole-trader entity + sample chart of accounts + sample journal entries spanning one FY. User can explore freely; switching back to `/` returns them to their real data unchanged. CRITICAL: namespace isolation is the only safe mitigation for demo data overwriting real user data. *Foundation locked 2026-06-02 (Plan 14-1):* `LocalAdapter` constructor widened (dbName param + DB_NAME_PROD/DB_NAME_DEMO exports), `seedDemoData()` helper ships the 1 sole-trader + 10-account COA + 15 balanced FY2025-26 journals, `initAdapter()` reads `getRouteKind()` and routes /demo → DB_NAME_DEMO with idempotent seed, and `demo-isolation.test.ts` locks PITFALLS §4 HARD-BLOCK via 3 cross-contamination tests. Requirement closes after Plan 14-2 ships the visible `<DemoModeBanner />` (top-of-app blue tint + "Exit demo" → `window.location.href = '/'` full reload).
...
- [x] **POL-04**: README rewritten: top-of-fold "Try the live demo at {URL}" + 1-line elevator pitch + screenshot. Quick-start section: (1) try the demo; (2) clone + self-host. Self-host section explains `npm install && npm run build && serve dist/` AND `npm run dev:full` for the Express+SQLite small-firm shape. Privacy footer points at `/privacy` page. Apache 2.0 + CONTRIBUTING link. *Closed 2026-06-02 (Plan 14-3):* README restructured 82 → 102 lines with new top-of-fold (1-line elevator pitch + live-demo URL + `> _Screenshot coming v1.3._` italic blockquote placeholder per CONTEXT discretion item 8), Quick Start reordered to `### Option 1: Try the demo` + `### Option 2: Clone and self-host`, Deployment Shapes header corrected to "three shapes", Optional AI section annotated with honest v5-deferral language aligned verbatim with PrivacyPage AI bullet (Plan 14-2 Task 3), new standalone `## Privacy` H2 section linking to `https://aussieledger.techtaitan.com/privacy`, all 7 pre-existing FND-12 test-locked phrases preserved byte-identically. `src/__tests__/readme.test.ts` extended with 7 new POL-04 content-presence assertions (live-demo URL, /demo, /privacy, Privacy H2 multi-line regex, "planned for v5", "Try the demo", ≥ 100 lines). 14 GREEN total in readme.test.ts; +7 SPA tests from this plan. Real screenshot deferred to v1.3 per CONTEXT discretion (locked).
...
| PWA-01 | Phase 13 | Complete (13-1 + 13-2 done 2026-06-02: install path + UpdateBanner + useUpdateBanner hook + user-verified Lighthouse PWA audit Installable + PWA Optimized both PASS; npm-run-dev SW absence verified; end-to-end UpdateBanner update flow verified incl. B-1 sessionStorage-persists-within-tab lock) |
| POL-01 | Phase 14 | Pending |
| POL-02 | Phase 14 | Pending |
| POL-03 | Phase 14 | Pending |
| POL-04 | Phase 14 | Complete (Plan 14-3, 2026-06-02) |
