...
must_haves:
  truths:
    - "Pushing a commit to main triggers .github/workflows/ci.yml; the existing `ci` job builds + lints + tests; a new `deploy` job (gated on `needs: ci` + `if: github.ref == 'refs/heads/main' && github.event_name == 'push'`) runs after CI passes and deploys dist/ to Cloudflare Pages within the workflow run â€” no manual step required (HOST-01 acceptance criterion 2)"
    - "The CI job widens to include a post-build AIza scan step (`grep -rE 'AIza[0-9A-Za-z_-]{35}' dist/ && exit 1 || true`) placed AFTER `npm run build` and BEFORE the artifact upload â€” failing scan fails CI and blocks deploy (HOST-02 acceptance criterion)"
    - "The CI job uploads dist/ via actions/upload-artifact@v4 after the scan passes; the deploy job downloads dist/ via actions/download-artifact@v4 as its first step â€” the deploy ships the exact bytes CI tested (no rebuild in deploy)"
    - "Deploy job sets `env: VITE_HOSTED_MODE: 'true'` BEFORE the build step of the CI job (since the artifact passed to deploy is the build output) â€” this means the build step in `ci` job has `VITE_HOSTED_MODE: 'true'` set when running on push-to-main, so the bundled SPA reads `import.meta.env.VITE_HOSTED_MODE === 'true'` and isHostedMode() returns true at runtime (HOST-03 acceptance criterion 4)"
    ...
    - "Navigating directly to `https://aussieledger.pages.dev/journals` from a fresh browser tab serves the SPA, not a 404 page â€” the `_redirects` `/* /index.html 200` fallback (created by 10-1) is confirmed working in production (HOST-01 acceptance criterion 1)"
    - "Running `curl -sI https://aussieledger.pages.dev` returns a 200 response containing the `Content-Security-Policy` header with `connect-src 'self' https://generativelanguage.googleapis.com` â€” confirms Cloudflare is applying `_headers` at the edge (HOST-01 acceptance criterion 3)"
  ...
  artifacts:
    ...
    - path: "README.md"
      provides: "New 'Self-host your own deploy' section after the existing self-host docs â€” concise (â‰¤ 30 lines), documents Cloudflare Pages prerequisite, secret names, workflow file path. Not the full README rewrite (POL-04, Phase 14)"
      contains: "Self-host your own deploy"
  key_links:
    - from: ".github/workflows/ci.yml ci job â€” AIza scan step"
    ...
    - from: ".github/workflows/ci.yml ci job â€” upload-artifact step"
      to: ".github/workflows/ci.yml deploy job â€” download-artifact step"
    ...
    - from: ".github/workflows/ci.yml deploy job â€” wrangler-action step"
    ...
    - from: ".github/workflows/ci.yml ci job â€” build step env"
...
**Manual prerequisites the USER must complete BEFORE running this plan (NOT planner tasks â€” flagged here for visibility):**
1. User generates a fine-grained Cloudflare API token at `dash.cloudflare.com/profile/api-tokens` â†’ "Create Custom Token" â†’ permissions `Account > Cloudflare Pages > Edit` + `Account > Account Settings > Read` + Zone Resources restricted to the `aussieledger` project.
2. User adds two secrets to GitHub repo Settings â†’ Secrets and variables â†’ Actions:
   - `CLOUDFLARE_API_TOKEN` (the token from step 1)
   - `CLOUDFLARE_ACCOUNT_ID` (from Cloudflare dashboard right sidebar)
3. User confirms the Cloudflare Pages project named `aussieledger` exists in the dashboard (per CONTEXT, project is pre-created â€” no Cloudflare-side creation task).
...
@.planning/phases/10-public-build-ci-cd-to-cloudflare-pages/10-1-PLAN.md
...
<interfaces>
<!-- Concrete contracts for the executor â€” no codebase exploration needed. -->

Existing `.github/workflows/ci.yml` (32 lines â€” verified at planning time, this is what we EXTEND, not replace):
```yaml
name: CI
...
Files Plan 10-1 created that this plan consumes (DO NOT MODIFY â€” they are already shipped by Wave 1):
- `src/lib/env.ts` â€” isHostedMode() helper. Phase 10-2 needs `VITE_HOSTED_MODE=true` in build env so the bundled value reads as 'true' at runtime.
- `public/_redirects` â€” Vite copies to dist/_redirects; Cloudflare applies SPA fallback.
- `public/_headers` â€” Vite copies to dist/_headers; Cloudflare applies CSP + security headers.
- `__fixtures__/aiza-secret-leak.txt` â€” lives OUTSIDE dist/, never scanned. Exists for unit-test verification only.
- `__fixtures__/__tests__/aiza-regex.test.ts` â€” verifies the regex shape this plan's CI scan uses.

AIza scan command (locked in 10-CONTEXT.md â€” placement: in `ci` job, AFTER `npm run build`, BEFORE `upload-artifact`):
```bash
grep -rE 'AIza[0-9A-Za-z_-]{35}' dist/ && exit 1 || true
```
The shell logic: `grep ... && exit 1` runs `exit 1` ONLY if grep found a match (exit 0). `|| true` makes the step exit 0 if grep found NO match (grep returns exit 1 for no-match â€” which we want to mean "scan clean"). The result: step fails iff a real AIza-shape string is in dist/.
...
<task type="checkpoint:human-action" gate="blocking">
  ...
  <what-needed>
    Three setup items only the user can do (Cloudflare dashboard + GitHub repo settings â€” no CLI/API available to Claude for these without already having the token we are creating):
    ...
    1. **Fine-grained Cloudflare API token created.**
       ...
       - Account Resources: Include â†’ `<your account>` (the one that owns the `aussieledger` Pages project)
       ...
       - Copy the token value (shown ONCE â€” if you navigate away, you must regenerate)

    2. **Two GitHub Secrets added** at `https://github.com/tech-taitan/AussieLedger/settings/secrets/actions`:
       - `CLOUDFLARE_API_TOKEN` â€” value from step 1
       - `CLOUDFLARE_ACCOUNT_ID` â€” visible in the right sidebar of any page in your Cloudflare dashboard (a 32-char hex string)

    3. **Cloudflare Pages project `aussieledger` exists.**
       - Per 10-CONTEXT.md, the project is pre-created. Verify at `https://dash.cloudflare.com/<account-id>/pages` â€” you should see a project named exactly `aussieledger` (lowercase, no spaces). Default URL: `https://aussieledger.pages.dev`.
       - If the project does NOT exist: in the Pages dashboard, click "Create application" â†’ "Pages" â†’ "Upload assets" â†’ name it exactly `aussieledger` â†’ upload a placeholder (a single `index.html` containing "hello" is fine â€” the first real deploy from CI will overwrite it). Direct-upload mode (not git-connected) is correct since GitHub Actions handles the trigger.
  ...
  <resume-signal>Type "ready" (token created + both secrets added + project confirmed) or "skip" (defer the actual deploy â€” proceed with workflow file changes but expect the first deploy attempt to fail until secrets are added)</resume-signal>
</task>

<task type="auto">
  <name>Task 2: Extend .github/workflows/ci.yml â€” widen ci job + add deploy job (HOST-01, HOST-02, HOST-03)</name>
  <files>
    .github/workflows/ci.yml
  </files>
  <read_first>
    - .github/workflows/ci.yml (current 32-line file â€” full content shown in <interfaces> above)
    - .planning/phases/10-public-build-ci-cd-to-cloudflare-pages/10-CONTEXT.md (Â§ In scope â€” full deploy job specification; Â§ Decisions â†’ all four sub-decision groups; Â§ Claude's Discretion â†’ Node 20 pin, wrangler-action v3 minor)
    - .planning/research/STACK.md Â§2 (Complete `.github/workflows/deploy.yml` skeleton â€” NOTE: that skeleton is for a SEPARATE deploy file; CONTEXT supersedes it with the locked decision to EXTEND ci.yml)
    - .planning/research/PITFALLS.md Â§1 (VITE_ env-leak HARD-BLOCK) + Â§5 (SPA routing 404) + Â§6 (CSP connect-src XSS defense) + Â§19 (Cloudflare base URL â€” `/` is correct for `aussieledger.pages.dev`, no Vite base config change)
    - .planning/phases/10-public-build-ci-cd-to-cloudflare-pages/10-1-PLAN.md (Plan 10-1 â€” confirms `public/_redirects` + `public/_headers` + `src/lib/env.ts` already exist by Wave 1)
  </read_first>
  <action>
        ...
        steps:
          ...
          - name: Build
            ...
            env:
              ...
              # pull_request) â€” both production deploys (push to main) AND PR preview
              # deploys (pull_request â†’ pr-{N}.aussieledger.pages.dev) ship the SAME
              ...
              # locally without VITE_HOSTED_MODE in the env â€” do NOT branch this CI step.
              # NEVER add a VITE_-prefixed SECRET here â€” only VITE_HOSTED_MODE (mode flag, not secret).
              # See .planning/research/PITFALLS.md Â§1 â€” VITE_ env-leak HARD-BLOCK.
              # src/lib/env.ts module doc mirrors this contract â€” keep them in sync.
          ...
          - name: Scan dist/ for accidentally-leaked Gemini API keys (HOST-02)
            ...
            # Pattern: standard Gemini key shape â€” 'AIza' + 35 chars from [0-9A-Za-z_-] = 39 chars.
            # Regex verified by __fixtures__/__tests__/aiza-regex.test.ts (Plan 10-1).
            # See .planning/research/PITFALLS.md Â§1 + 10-CONTEXT.md "AIza secret-leak scan placement".
            run: |
              if grep -rE 'AIza[0-9A-Za-z_-]{35}' dist/; then
                echo "::error::AIza-shape string found in dist/ â€” possible Gemini API key leak. Build blocked."
                exit 1
              else
                echo "AIza scan clean â€” no Gemini-key-shape strings in dist/."
              fi
    ...
    **Critical structural notes the executor must NOT change:**
    ...
    1. **AIza scan placement.** The scan step lives in the `ci` job, AFTER `Build`/`Test`, BEFORE `Upload dist/ artifact`. This ordering guarantees: (a) the scan runs against the same `dist/` that gets uploaded, and (b) a failed scan blocks both the artifact upload AND the deploy job (because `deploy` has `needs: ci` â€” any CI step failure causes the whole `ci` job to fail, which prevents `deploy` from starting).

    2. **`VITE_HOSTED_MODE: 'true'` in build env.** Set ONLY on the `Build` step, as a string `'true'` (NOT bare `true` â€” YAML would parse that as boolean and Vite would expose `true` not `'true'`, breaking isHostedMode()'s strict string check). Setting it at the build step makes it visible to `vite build`; the value is then baked into the bundle via `import.meta.env.VITE_HOSTED_MODE`.
    ...
    4. **Two separate `wrangler-action` steps for production vs PR preview.** This is the simplest readable pattern given the `--branch` flag differs. An alternative would be a single step with conditional command construction â€” readability wins.
    ...
    6. **DO NOT add `workflow_dispatch:` triggers.** Locked in CONTEXT â€” every push to main auto-deploys; no manual hotfix path needed for v1.2 (acceptable risk given the 983-test CI gate).

    7. **DO NOT add a `Lighthouse CI` step.** Locked in CONTEXT â€” PWA Lighthouse audit lives in Phase 13 (PWA-01); Phase 10 doesn't need it.
  ...
  <done>
    ...
    - Two separate `wrangler-action@v3` steps exist â€” one gated by `if: github.ref == 'refs/heads/main' && github.event_name == 'push'`, the other by `if: github.event_name == 'pull_request'`
...
<task type="auto">
  ...
  <read_first>
    - README.md (existing content â€” find a logical insertion point near the existing self-host / clone-and-run docs)
    - .planning/phases/10-public-build-ci-cd-to-cloudflare-pages/10-CONTEXT.md (Â§ In scope â€” "README addition: 'Self-host your own deploy' section documenting the Cloudflare Pages prerequisite, secret names, and the workflow file location. Not the full hosting-mode README rewrite (that's POL-04 in Phase 14); just the deploy infra reference")
    - .planning/REQUIREMENTS.md POL-04 (the FULL README rewrite â€” explicitly DEFERRED to Phase 14; do NOT do that work here)
  </read_first>
  <action>
    ...
    **Step 2: Insert this EXACT content as a new section** (concise â€” â‰¤ 30 lines per CONTEXT decision; full audience-first README rewrite is POL-04 in Phase 14):
    ...
    1. **A Cloudflare Pages project** named `aussieledger` (or rename it and update `--project-name=` in `.github/workflows/ci.yml`). Create it in the [Cloudflare Pages dashboard](https://dash.cloudflare.com) â†’ "Create application" â†’ "Pages" â†’ "Upload assets" (direct-upload mode; not git-connected â€” GitHub Actions handles the trigger).

    2. **A fine-grained Cloudflare API token** scoped to the Pages project. Create at [dash.cloudflare.com/profile/api-tokens](https://dash.cloudflare.com/profile/api-tokens) â†’ "Create Custom Token" with permissions `Account > Cloudflare Pages > Edit` + `Account > Account Settings > Read`.

    3. **Two GitHub Secrets** on your fork (Settings â†’ Secrets and variables â†’ Actions):
       - `CLOUDFLARE_API_TOKEN` â€” the token from step 2
       - `CLOUDFLARE_ACCOUNT_ID` â€” visible in the Cloudflare dashboard's right sidebar
    ...
    The pipeline includes a defensive `AIza` scan that blocks any build where a Gemini API key shape ends up in `dist/` â€” see [PITFALLS.md Â§1](./.planning/research/PITFALLS.md) for the security rationale. Never set `VITE_GEMINI_API_KEY` (or any `VITE_`-prefixed secret) in your CI environment.
    ```

    **Step 3: Verify line count.**

    The new section should be â‰¤ 30 lines (excluding blank lines around the heading). Per CONTEXT, this is a concise reference, not the full README rewrite (POL-04 in Phase 14 does the audience-first restructure).
  ...
  <done>
    ...
    - Section is â‰¤ 30 content lines (concise reference, not full rewrite)
...
<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 4: Post-deploy verification â€” confirm production deploy + CSP + SPA routing + security grade</name>
  <what-built>
    Tasks 1-3 have:
  ...
  <how-to-verify>
    ...
    **Verification steps (run all five â€” pass all or report which failed):**
    ...
    3. **CSP headers applied at edge (HOST-01 acceptance criterion 3).** Run from your terminal:
       ...
       ```
       Expected output: a `content-security-policy:` line containing `default-src 'none'`, `script-src 'self'`, and `connect-src 'self' https://generativelanguage.googleapis.com`. If empty: `public/_headers` was not picked up â€” investigate.

    4. **Security headers grade.** Visit `https://securityheaders.com/?q=https%3A%2F%2Faussieledger.pages.dev&followRedirects=on`. Expected grade: **A** or **A+**. Report the grade and the exact headers panel. Acceptable failures: a `B` is acceptable on first pass if the only docked item is the `Permissions-Policy` (some scanners disagree on the modern syntax). A `C` or below should be reported as "issues" â€” the policy needs revision.

    5. **Browser console clean on production.** Open `https://aussieledger.pages.dev/` in Chrome or Firefox with DevTools open (F12). Reload once. Console shows ZERO red error messages mentioning `Content Security Policy`, `Refused to load`, `Refused to apply inline style`, or `violates the following Content Security Policy directive`. The app renders fully. Navigate to two other routes (e.g. `/journals`, `/trial-balance` if those are the actual paths) â€” each renders cleanly.
    ...
    **If deploy FAILED at CI:**
    - "AIza scan blocked" â†’ a real Gemini key shape was found in `dist/`. STOP â€” do not bypass. Investigate which build output contains it (likely a contributor accidentally set `VITE_GEMINI_API_KEY` in some env). Fix the source, then push again. This is the intended HARD-BLOCK behavior of HOST-02.
    - "Cloudflare authentication failed" â†’ the API token is invalid or missing. Re-verify Task 1 prerequisites (token still valid, both secrets set with correct names spelled `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`).
    - "Project not found" â†’ the Cloudflare Pages project name doesn't match `aussieledger`. Either rename the project to `aussieledger` in the Cloudflare dashboard OR edit `.github/workflows/ci.yml` to match your actual project name and push the fix.
...
- `https://aussieledger.pages.dev/journals` returns the SPA (not 404) â€” proving `_redirects` works end-to-end (Task 4 step 2).
...
- v1.1 baseline preserved: 983 SPA GREEN + 15 new tests from Plan 10-1 â†’ 998+ SPA GREEN; 18 server GREEN; lint EXIT 0; build EXIT 0. Plan 10-2 adds zero new tests (it is a workflow + docs plan, no new source code).
...
- `VITE_HOSTED_MODE` is the only `VITE_`-prefixed env var in the CI environment â€” verified by inspection of the workflow file; no `VITE_GEMINI_API_KEY` or similar secret-shaped var exists anywhere.
</verification>
...
- HOST-03 fully satisfied: `VITE_HOSTED_MODE` flag set to `'true'` on the public Cloudflare build (criterion 1); single source of truth via `isHostedMode()` helper in `src/lib/env.ts` (criterion 2 â€” already shipped by Plan 10-1); ready for Phase 12 (`AiGateNote` consumption) and Phase 13 (PWA registration gate). The acceptance criterion 4 ("`/api/health` probe is skipped on startup, and `AiGateNote` renders the hosted-mode copy") is partially satisfied â€” the env-var pipeline works, but the actual `/api/health` skip + `AiGateNote` hosted-mode link are wired in Phase 12.
...
- Custom domain (HOST-04 â†’ Phase 14; v1.2 ships on `aussieledger.pages.dev` for the first 1-2 weeks)
...
- Touching StorageAdapter (FINAL invariant â€” Phase 10 has zero adapter changes)
